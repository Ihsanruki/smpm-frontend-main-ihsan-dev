...
const ReportPDF = ({ data }: { data: ReportPDFProps }) => {  
    ...
    <Document>  
      <Page size="A4" style={{ padding: 20, fontSize: 10 }}>  
        ...
        <View style={{ marginBottom: 20, flexDirection: "row", justifyContent: "space-between" }}>  
          <View>  
            {addPlaceholders([  
              ...
              { label: "Status Approval:", value: displayValue(data.status_approve), color: data.status_approve?.toLowerCase() === "pending" ? "blue" : data.status_approve?.toLowerCase() === "rejected" ? "red" : "green" },  
          ...
          <View>  
            {addPlaceholders([  
              ...
              { label: "SIM Card Provider:", value: displayValue(data.simcard_provider) },  
            ]).map((item, index) => (  
              <View key={index} style={{ flexDirection: "row", marginBottom: 5 }}>  
                <Text style={{ fontWeight: "bold" }}>{item.label}</Text>  
                <Text style={{ marginLeft: 5 }}>{item.value}</Text>  
        ...
        <View style={{ marginTop: 20 }}>  
          <Text style={{ fontWeight: "bold", marginBottom: 5 }}>Images</Text>  
          <View style={{ flexWrap: "wrap", flexDirection: "row" }}>  
            {data.images.map((image, index) => (  
              <Image key={index} src={image.media.path} style={{ width: "33.33%", height: "auto", marginBottom: 10 }} />  
            ))}  
          </View>  
        </View>  
      </Page>  
