...
const TableVendor: React.FC = () => {  
  const { tableFilter, onChangeTable } = useTableHelper<IVendorModel>();  
  ...
  const {  
    ...
    queryKey: ["activity-job-order", { ...tableFilter, searchValue, currentPage, pageSize }],  
    queryFn: () =>  
      getActivityJobOrder({  
  ...
  const columns: ColumnsType<IVendorModel> = useMemo(  
    (): ColumnsType<IVendorModel> => [  
      {  
        title: "No. JO",  
        dataIndex: "name",  
        sorter: true,  
